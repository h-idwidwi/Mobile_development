...
class FoodByCategories extends StatefulWidget {
  final dynamic categoryId;
  const FoodByCategories(this.categoryId, {Key? key}) : super(key: key);
  @override
  State<StatefulWidget> createState() => FoodByCategoriesState();
}

class FoodByCategoriesState extends State<FoodByCategories> {
  late Box<Favorites> favoritesBox;

  Future<List<Map<String, dynamic>>> getFoodByCategory(categoryId) async {
    await Supabase.instance.client.auth.signInWithPassword(email: 'hidwidwi@mail.ru', password: 'azUGyX278');
    final response = await Supabase.instance.client.from('food').select().eq('category', int.parse(categoryId));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        future: getFoodByCategory(widget.categoryId), 
